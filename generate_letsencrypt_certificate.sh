...
if [[ ! -f "generate_letsencrypt_certificate.sh" ]]; then
    echo ""
...
certbot -v certonly --agree-tos --register-unsafely-without-email --webroot -w $CWD/data/letsencrypt-challenge -d $domain \
  --deploy-hook "cp /etc/letsencrypt/live/$domain/fullchain.pem $CWD/config/certificate_chain.crt; \
                 cp /etc/letsencrypt/live/$domain/fullchain.pem $CWD/data/smtpcert/server.crt; \
                 cp /etc/letsencrypt/live/$domain/privkey.pem $CWD/config/private.key; \
...
echo ""
echo "* * */30 * * cd /root/edcom-install && ./renew_letsencrypt_certificate.sh $domain > data/logs/certbot.log 2>&1"
echo ""
echo ""
