...
certbot -v renew --force-renewal --cert-name $1 --webroot -w $CWD/data/letsencrypt-challenge \
        --deploy-hook "cp /etc/letsencrypt/live/$1/fullchain.pem $CWD/config/linkcerts/$1.certificate_chain.crt; \
                       cp /etc/letsencrypt/live/$1/privkey.pem $CWD/config/linkcerts/$1.private.key; \
                       $CWD/restart.sh"
