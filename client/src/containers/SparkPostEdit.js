...
class SparkPostEdit extends Component {
  handleChange = event => {
  ...
  handleSubmit = async event => {
    ...
    if (isclose) {
      ...
      this.props.history.replace("/sparkpost/edit?id=" + id);
    }
  }

  goBack = () => {
    this.props.history.push("/sparkpost");
  }
  ...
  navbarButtons = () => {
    return (
      <LoaderButton
        ...
        onClick={() => {
          sendGA4Event('Connections', 'SparkPost', 'Saved SparkPost Configuration');
          return this.props.formSubmit(true);
        }}
        splitItems={[
          { text: 'Save', onClick: () => { sendGA4Event('Connections', 'SparkPost', 'Saved SparkPost Configuration'); return this.props.formSubmit(); } },
  ...
  render() {
    ...
    var title = `${this.props.id === 'new'?'Add SparkPost Account':`Edit SparkPost Account ${dataName ? `for "${dataName}"` : ''}`}`;
          ...
          <EDFormSection onSubmit={this.handleSubmit} formRef={this.props.formRef}>
            <EDFormBox>
              ...
              <SelectLabel
                ...
                options={regions}
                emptyVal="SparkPost"
                help="Select SparkPost EU if your SparkPost account is in the EU region"
                space
              />
              <FormControlLabel
                id="linkdomain"
...
export default withLoadSave({
  extend: SparkPostEdit,
  initial: { name: '', apikey: '', linkdomain: '', region: '' },
  get: async ({id}) => (await axios.get('/api/sparkpost/' + id)).data,
  post: ({data}) => axios.post('/api/sparkpost', data),
  patch: ({id, data}) => axios.patch('/api/sparkpost/' + id, data),
});
