...
class IPDelivery extends Component {
  constructor(props) {
    super(props);
  ...
  saveConfig = () => {
    axios.patch('/api/tableconfigs/ipdelivery', {show: this.state.show});
  }
  ...
  render() {
    let columns = [{
      ...
      Cell: ({...props}) => {
        ...
        if (special.find(s => s === o.sinkid)) {
          ...
          return <Link to={'/servers/stats?id=' + o.sinkid + '&domaingroupid=' + o.domaingroupid + '&settingsid=' + o.settingsid +
                            '&ip=' + o.ip + '&returnto=ipdelivery&tablestate=' + this.tableState()}>{props.value}</Link>
        }
      },
      mandatory: true,
    }, {
      ...
      Cell: ({...props}) => {
        ...
        if (props.value === null || special.find(s => s === o.sinkid)) {
          ...
          return <Link to={'/servers/stats?id=' + o.sinkid + '&domaingroupid=' + o.domaingroupid + '&settingsid=' + o.settingsid +
                            '&ip=' + o.ip + '&returnto=ipdelivery&tablestate=' + this.tableState()}>{props.value.toLocaleString()}</Link>
        }
      },
      className: 'text-right',
      ...
      Cell: ({...props}) => {
        if (!props.value) {
          ...
          var {start, end} = this.state;
          return <Link to={'/statmsgs?type=defer' +
                           '&returnto=' + encodeURIComponent('/ipdelivery?tablestate=' + this.tableState()) +
                           ...
                           }>{props.value.toLocaleString()}</Link>
        }
      },
      className: 'text-right',
      mandatory: true,
    }, {
      Header: 'Deferral Ratio',
      headerClassName: 'text-right',
      accessor: 'deferratio',
      Cell: ({...props}) => <span className={props.original.txtcls}>{props.value.toFixed(2) + '%'}</span>,
      className: 'text-right',
      show: this.showColumn('deferratio'),
    }, {
      Header: 'Errors',
      headerClassName: 'text-right',
      accessor: 'err',
      Cell: ({...props}) => {
        if (!props.value) {
          ...
          var {start, end} = this.state;
          return <Link to={'/statmsgs?type=err' +
                           '&returnto=' + encodeURIComponent('/ipdelivery?tablestate=' + this.tableState()) +
                           '&start=' + dateformat(start) +
                           '&end=' + dateformat(end) +
                           '&domaingroupid=' + props.original.domaingroupid +
    ...
    let data = _.filter(_.map(this.state.data, s => {
      ...
      return {
        ...
        deferratio: pct(s.defercnt, s.send+s.defercnt+s.err+s.soft+s.hard),
    ...
    return (
      <MenuNavbar {...this.props} isAdmin={true}>
        <TitlePage title="IP Delivery Report" tabs={
          <EDTabs>
            <Nav className="nav-tabs space15" activeKey="1">
              <NavItem eventKey="1" disabled>Delivery</NavItem>
              <NavItem eventKey="2" onClick={this.switchView.bind(null, "/ipreputation")}>Reputation</NavItem>
            </Nav>
          </EDTabs>
...
export default withLoadSave({
  extend: QueueSelection(IPDelivery),
  initial: [],
  get: async () => [],
  extra: {
    ...
    tableconfig: async () => (await axios.get('/api/tableconfigs/ipdelivery')).data,
  }
});
