...
class GalleryTemplates extends Component {
  constructor(props) {
    super(props);
  ...
  createClicked = () => {
    this.props.history.push("/gallerytemplates/edit?id=new");
  }

  deleteConfirmClicked = async id => {
    await axios.delete('/api/gallerytemplates/' + id);
    await this.props.reload();
  }

  duplicateClicked = async id => {
    await axios.post('/api/gallerytemplates/' + id + '/duplicate');
    await this.props.reload();
  }
    ...
    return (
      <div className="servers">
        <MenuNavbar {...this.props} isAdmin={true}>
          <TitlePage title="Template Gallery (Legacy)" button={
            ...
            <EDTabs>
              <Nav className="nav-tabs space15" activeKey="3">
                ...
                <NavItem eventKey="3" disabled>Template Gallery (Legacy)</NavItem>
          ...
          <LoaderPanel isLoading={this.props.isLoading}>
            {this.props.data.length ?
                  ...
                  <EDCard key={s.id} header={
                    <div>
                      ...
                      <h3 className="server-name">
                        <Link to={'/gallerytemplates/edit?id=' + s.id}>
                          {s.name}
                        </Link>
                      </h3>
                    </div>
                    ...
                    <Row>
                      ...
                      <Col xs={12} className="text-center space25">
                        <ConfirmDropdown
                          ...
                          extra={true}
                          text="Actions">
                          <MenuItem onClick={() => this.props.history.push('/gallerytemplates/edit?id=' + s.id)}>Edit</MenuItem>
              ...
              <div className="text-center space-top-sm">
                <h4>No broadcast templates configured.</h4>
                <h5>Broadcast templates appear as selections when users create a new message.</h5>
              </div>
            }
          </LoaderPanel>
        </MenuNavbar>
...
export default withLoadSave({
  extend: GalleryTemplates,
  initial: [],
  get: async () => _.sortBy((await axios.get('/api/gallerytemplates')).data, s => s.name.toLowerCase())
});
