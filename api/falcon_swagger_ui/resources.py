...
class TemplateRenderer(object):
    ...
    def _load_template(self, tpl):
        ...
        return jinja2.Environment(
            loader=jinja2.FileSystemLoader(os.path.join(path, templates_directory))
...
class StaticSinkAdapter(object):
    ...
    def __call__(self, req, resp, filepath):
        ...
        curr_dir = os.path.dirname(os.path.abspath(__file__))
        file_path = os.path.join(os.path.join(curr_dir, self.static_path), filepath)
...
def register_swaggerui_app(
    app, base_url, api_url, page_title="Swagger UI", favicon_url=None, config=None
):
    """:type app: falcon.API"""
    ...
    app.add_route(base_url, SwaggerUiResource(templates_folder, default_context))
