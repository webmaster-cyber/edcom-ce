...
if [[ ! -f "renew_letsencrypt_certificate.sh" ]]; then
    echo ""
...
certbot -v renew --force-renewal --cert-name $1 --webroot -w $CWD/data/letsencrypt-challenge \
	--deploy-hook "cp /etc/letsencrypt/live/$1/fullchain.pem $CWD/config/certificate_chain.crt; \
                       cp /etc/letsencrypt/live/$1/fullchain.pem $CWD/data/smtpcert/server.crt; \
                       cp /etc/letsencrypt/live/$1/privkey.pem $CWD/config/private.key; \
                       cp /etc/letsencrypt/live/$1/privkey.pem $CWD/data/smtpcert/server.key; \
                       $CWD/restart.sh"
